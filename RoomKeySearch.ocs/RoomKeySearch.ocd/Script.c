...
public func GetRoomSection() { return "KeySearch"; }
...
public func GetRoomID() { return "KS"; }

// The difficulty of this room, ranges from 0 to 999.
public func GetRoomDifficulty() { return 15; }
...
{
	// Create key and lock the exit with it.
	var container_key = FindObject(Find_Property("IsKeyContainer"), Sort_Random());
	var key = container_key->CreateContents(Key);
	key.SaveScenarioObject = Global.NoSave;
	var exit = FindObject(Find_ID(RoomExit));
	exit->MakeLockedDoor(key);
	// Randomly place the tablet.
	var container_tablet = FindObject(Find_Property("IsKeyContainer"), Find_Exclude(container_key), Sort_Random());
	var tablet = container_tablet->CreateContents(AncientTablet);
	tablet.SaveScenarioObject = Global.NoSave;
	// Add fake keys to all other containers.
	for (var container in FindObjects(Find_Property("IsKeyContainer"), Find_Exclude(container_key), Find_Exclude(container_tablet)))
	{
		var key = container->CreateContents(Key);
		key->SetColor(HSL(Random(256), 255, 100 + Random(60)));
		key.SaveScenarioObject = Global.NoSave;
	}
	return;
}
