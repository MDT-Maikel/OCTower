...
public func GetRoomID() { return "SG"; }

// The difficulty of this room, ranges from 0 to 999.
public func GetRoomDifficulty() { return 181; }
...
public func GetRoomHints() { return ["$RoomHint1$"]; }
...
{
	RemoveAll(Find_ID(RoomColoredClouds));
	Fern->Place(20);
	Mushroom->Place(20);
	Flower->Place(50);
}
...
{	
	var knowledge = [SteamEngine, Flagpole, Basement, Idol, Compensator, Kitchen, Pump, Sawmill, Chest, Armory, Loom, ToolsWorkshop, Windmill, WindGenerator,
		Cannon, Catapult, Lorry, Bread, Flour, Cloth, Axe, Hammer, Barrel, MetalBarrel, Crate, Lantern, Pipe, Shovel, Sickle, Pickaxe, DivingHelmet, 
		Bow, BombArrow, FireArrow, Arrow, Blunderbuss, LeadBullet, Helmet, Club, SmokeBomb, Shield, Sword, Javelin];
	for (var know in knowledge)
	{
		SetPlrKnowledge(plr, know);
	}
}
